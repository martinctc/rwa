...
The `use` and `weight` arguments apply to **multiple regression only**. Neither
affects a logistic fit. `rwa()` warns when you supply a weight column or a
non-default `use` value alongside logistic RWA, and ignores them, because
`rwa_logit()` does not fit a weighted or correlation-based model:

```{r rwa-multiple-only, eval=FALSE}
...
See `vignette("weighted-missing-data", package = "rwa")` for weighted
estimation and the missing-data contract.
